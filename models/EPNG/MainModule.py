...
class MainModule(nn.Module):
    def __init__(self, mask_layer, decoder_layer) -> None:
        ...
        self.vs_backbone = resnet_fpn_backbone('resnet101', pretrained=True, trainable_layers=2)
    ...
    def forward(self, image, text, noun_index):
        ...
        text_mask = torch.full((b, 8, 1, text.shape[1]), True).to(device)

        for temp in range(b):

            max_value = int(noun_index[temp].cpu().numpy().max())
            text_mask[temp, :, :, 1: max_value + 1] = False
        

        image_layer = self.encoder(image_layer.view(b, -1, h*w).permute(0, 2, 1), text, None, text_mask)
