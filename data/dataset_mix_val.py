...
class PTData(Dataset):
    def __init__(self, cfg, train) -> None:
        ...
        self.annotation_path = osp.join(cfg.DATA.ANNO_PATH, '{:s}.json'.format(self.split))
    ...
    def __getitem__(self, indexes):
        ...
        image = Image.open(osp.join(self.image_path, annotation['file_name']))
        ...
        segms = torch.zeros((self.num_box, annotation['image_h'], annotation['image_w']))
        noun_index = torch.IntTensor(np.zeros((self.num_box)))
        single = torch.IntTensor(np.zeros((self.num_box)))
        thing = torch.IntTensor(np.zeros((self.num_box)))
        ...
        for i in annotation['segments']:
            ...
            if is_thing:
                single[pos - 1] = single[pos - 1] + 1
            thing[pos - 1] = is_thing

        segms[segms > 0] = 1
        segms.requires_grad = False
                # input_ids.requires_grad = False

        return image, segms, noun_index, gt_noun, thing, single, annotation['image_h'], annotation['image_w']
